...
if [[ "${SSL_ENABLE,,}" == 'true' ]] ; then
    ...
    if [[ -n "$SSL_KEY" ]]; then
        echo "$SSL_KEY" | base64 -D | sudo tee /etc/contrail/ssl/private/server-privkey.pem > /dev/null
    fi
    if [[ -n "$SSL_CERT" ]]; then
        echo "$SSL_CERT" | base64 -D | sudo tee /etc/contrail/ssl/certs/server.pem > /dev/null
    fi
    if [[ -n "$SSL_CACERT" ]]; then
        echo "$SSL_CACERT" | base64 -D | sudo tee /etc/contrail/ssl/certs/ca-cert.pem > /dev/null
    fi
fi
