...
function install_docker_rhel() {
    which docker && return
    local local_docker=$([["$RHEL_VERSION" == "rhel8" ]] && echo "podman podman-docker" || echo "docker")
    yum install -y $local_docker device-mapper-libs device-mapper-event-libs
    [ "$RHEL_VERSION" == "rhel8" ] && sudo alternatives --set docker /usr/bin/docker
}
...
if ! which docker >/dev/null 2>&1 ; then
    if [ x"$DISTRO" == x"centos" ]; then
        ...
        systemctl stop firewalld || true
        install_docker_rhel
        "$RHEL_VERSION" == "rhel8" || systemctl start docker
...
DOCKER_CONFIG=$([[ "$RHEL_VERSION" == "rhel8" ]] && echo "/etc/containers/registries.conf" || [[ -z $INSECURE_REGISTRIES ]] && echo "/etc/docker/daemon.json" || echo "/etc/sysconfig/docker")
...
EOF
    ...
    INSECURE_REGISTRIES+="'$CONTAINER_REGISTRY'"
    INSECURE_REGISTRIES=$(echo "$INSECURE_REGISTRIES" | sed 's/ \+/ /g')
    INSECURE_REGISTRIES=$(echo "$INSECURE_REGISTRIES" | sed 's/ /,/g')
    INSECURE_REGISTRIES="registries = [$INSECURE_REGISTRIES]"
    echo "INFO: new registries are $INSECURE_REGISTRIES"
    sudo sed "/registries.insecure/{n; s/registries = .*$/${INSECURE_REGISTRIES}/g}" ${DOCKER_CONFIG} > registries.conf.tmp
    sudo mv registries.conf.tmp ${DOCKER_CONFIG}
fi
