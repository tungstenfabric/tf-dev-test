...
if [[ -z "$INSECURE_REGISTRIES"]]; then
    CONFIG_FILENAME="/etc/docker/daemon.json"
else
    CONFIG_FILENAME="/etc/sysconfig/docker"
fi

if [[ -n "${CONTAINER_REGISTRY}" ]]; then
    if [ ! -e $CONFIG_FILENAME ]; then
        touch $CONFIG_FILENAME
    elif grep -q $CONTAINER_REGISTRY $CONFIG_FILENAME; then
        echo "Registry $CONTAINER_REGISTRY is configured in $CONFIG_FILENAME. Skip."
        exit 0
    fi
    
    if is_registry_insecure "$CONTAINER_REGISTRY"; then
        if [ "$CONFIG_FILENAME" == "/etc/sysconfig/docker" ]
