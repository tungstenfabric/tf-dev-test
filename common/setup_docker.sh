...
if ! is_registry_insecure "$CONTAINER_REGISTRY" && [ -n "$TF_TEST_IMAGE" ] && ! is_registry_insecure "$TF_TEST_IMAGE" ; then
    echo "both $CONTAINER_REGISTRY and $TF_TEST_IMAGE are secure"
    exit
fi
...
[ -z "$TF_TEST_IMAGE" ] || is_registry_insecure "$TF_TEST_IMAGE" || configure_insecure_registries_$DOCKER_CONFIG_DIR "$TF_TEST_IMAGE"
