...
function configure_insecure_registries_containers() {
    ...
    INSECURE_REGISTRIES="registries =[$(echo "$INSECURE_REGISTRIES'$CONTAINER_REGISTRY' " | sed 's/ \+/,/g')]"
...
is_registry_insecure "$CONTAINER_REGISTRY" || ! echo "${CONTAINER_REGISTRY} is secure. exit" || exit
...
# add CONTAINER_REGISTRIE into DOCKER_CONFIG
configure_insecure_registries_$(echo "$DOCKER_CONFIG" | cut -d "/" -f3 )
