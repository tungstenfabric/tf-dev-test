...
TF_TEST_INPUT_TEMPLATE=${TF_TEST_INPUT_TEMPLATE:-"$my_dir/deployment_test_input.yaml.j2"}
...
"$my_dir/../common/jinja2_render.py" < $TF_TEST_INPUT_TEMPLATE > ./deployment_test_input.yaml

echo "INFO: TF test input:"
cat ./deployment_test_input.yaml

ssl_opts=''
if [[ "${SSL_ENABLE,,}" == 'true' ]] ; then
    echo "INFO: SSL enabled. prepare certs from env if exist"
    sudo mkdir -p /etc/deployment/ssl/private /etc/deployment/ssl/certs
    if [[ -n "$SSL_KEY" ]]; then
        echo "$SSL_KEY" | base64 -d -w 0 | sudo tee /etc/deployment/ssl/private/server-privkey.pem > /dev/null
    fi
    if [[ -n "$SSL_CERT" ]]; then
        echo "$SSL_CERT" | base64 -d -w 0 | sudo tee /etc/deployment/ssl/certs/server.pem > /dev/null
    fi
    if [[ -n "$SSL_CACERT" ]]; then
        echo "$SSL_CACERT" | base64 -d -w 0 | sudo tee /etc/deployment/ssl/certs/ca-cert.pem > /dev/null
    fi
    ssl_opts='-l /etc/deployment/ssl'
fi
...
if HOME=$WORKSPACE ./testrunner.sh run -H \
    ...
    test_failures="$(grep testsuite ${WORKSPACE}/deployment-test-runs/*/reports/TESTS-TestSuites.xml | grep -o  'failures=\S\+' | uniq)"
    if [[ x"$test_failures" != x'failures="0"' ]]; then
        echo "ERROR: there were failures during the test."
        echo "       See detailed logs in ${WORKSPACE}/deployment-test-runs"
        exit 1
    fi
fi
